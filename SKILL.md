...
description: Use when the user asks for Discord course updates, summaries, action items, decisions, or recent activity from the configured AI Performance Engineering IL Discord channels. Uses the local discord_cli.py tool to check cache status, refresh only when needed, emit bounded evidence, and summarize in English with source-window caveats.
---
...
1. Resolve the user's request to a configured Discord channel using the request text and `python discord_cli.py --list-channels` if needed.
2. Run `python discord_cli.py --status` or `python discord_cli.py --status <channel>`.
3. Refresh one configured channel only when cache is `missing`, `stale`, `incomplete`, or when the user explicitly asks for latest/current updates:
   - `python discord_cli.py --refresh-channel <channel>`
   - Do not run `--bootstrap-server` for normal summaries.
4. Get evidence:
   - For a known channel: `python discord_cli.py --context <channel>`
   - For natural-language routing: `python discord_cli.py --summary-context "<user request>"`
...
- Prefer cached normalized data when it is fresh enough for the request.
